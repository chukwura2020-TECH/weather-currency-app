...
🐛 FIXED: Converter and History boxes now CENTERED and WIDER!
"""
import csv
...
class CurrencyConverter(tk.Frame):
    ...
    def _create_widgets(self):
        ...
        # ── CONVERTER CARD ── CENTERED & WIDER
        converter_container = tk.Frame(scrollable_frame, bg=COLORS['bg_primary'])
        converter_container.pack(fill="x", pady=5)

        # Use a centering trick: anchor the card in the middle of the container
        converter_card = tk.Frame(converter_container, bg='#FFFFFF')
        converter_card.pack(anchor="center", padx=30, fill="x")  # wider: 30px side padding only
        ...
        # ── HISTORY SECTION ── CENTERED & WIDER
        history_header = tk.Frame(scrollable_frame, bg=COLORS['bg_primary'])
        history_header.pack(fill="x", padx=30, pady=(15, 5))  # wider: 30px padding
        ...
        # History container - WIDER & CENTERED
        history_outer = tk.Frame(scrollable_frame, bg=COLORS['bg_primary'])
        history_outer.pack(fill="x", padx=30, pady=(0, 10))  # wider: 30px padding
    ...
    def _add_to_history(self, conversion_data):
        """Add to history - WIDER & CENTERED boxes"""
